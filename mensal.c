...
void inserirNaFila(Fila *fila, void *e) {
    ...
    fila->fim = (fila->fim + 1) % fila->capacidade; // Move o índice de fim para a frente
...
void* removerDaFila(Fila *fila) {
    ...
    void *e = fila->dados[fila->inicio]; // Armazena o elemento a ser removido
    fila->inicio = (fila->inicio + 1) % fila->capacidade; // Move o índice de início para a frente
